...
This project contains code to build a battery-powered E-Ink dashboard for the entire family based on an Inkplate 10. It shows the current week of a calendar feed which is rendered in a Docker container that runs somewhere in your home network. It started as a fork of the great [MagInkDash](https://github.com/speedyg0nz/MagInkDash) but the codebase has changed quite a bit since then.
...
On the backend, a Python API based on Docker and FastAPI is serving the image with all the desired info. As soon as the Inkplate requests the image, it pulls the calendar data. The retrieved content is then formatted into the desired layout and served as a PNG image file.
...
* **Calendar**: The dashboard displays the events of the current week (Sunday to Saturday) in a columnar layout, with one column per day. If you have a busy calendar with numerous events on a single day, the space in that column will be consumed quickly and long entries may be clipped.
